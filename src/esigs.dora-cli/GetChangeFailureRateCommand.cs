...
{
    public sealed class GetChangeFailureRateCommand : Command<GetChangeFailureRateCommand.Settings>
    {
        ...
        public override int Execute(CommandContext context, Settings settings)
        {
            ...
            var result = metricsTracker.GetChangeFailureRate(settings.StartDate, settings.EndDate);

            if (result.IsSuccess)
            {
                AnsiConsole.MarkupLine($"[green]Change Failure Rate: {result.Value}[/]");
                return 0;
            }
            else
            {
                AnsiConsole.MarkupLine($"[red]Error getting change failure rate: {result.Error}[/]");
                return -1;
            }
        }
    }
